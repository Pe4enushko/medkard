...
def test_clinical_picture_beats_epidemiology_on_a_real_toc():
    """Оглавление клинрека 290_2 из прогона 2026-08-23, как оно лежит в docs.

    Отбор по номеру подряд («первые пять из 1.x») ронял 1.6 всегда: у 24
    клинреков из 30 разделов 1.x ровно шесть, и лимит выбирался на 1.1–1.5.
    Судью при этом спрашивают, подходит ли код клинической картине пациента.
    """
    toc = [
    ...
    assert picked[0].startswith("1.6")
    assert not any(s.startswith("1.3") for s in picked)
    assert "Критерии оценки качества медицинской помощи" not in picked


def test_section_number_wins_over_a_name_mangled_by_pdf_parsing():
    """У клинрека 748_2 раздел зовётся «1.6 \\nКлиническая» — имя обрезано при
    разборе PDF, и по слову «картина» он не находится вовсе."""
    picked = icd._pick_sections([
        "1.1 Определение заболевания или состояния (группы заболеваний или",
...
def test_reading_order_is_usefulness_order_so_the_budget_cuts_the_least_needed():
    """Порядок отбора — он же порядок чтения: бюджет символов обязан кончаться
    на эпидемиологии, а не на клинической картине."""
    picked = icd._pick_sections([
        "1.2 Этиология и патогенез",
        "1.3 Эпидемиология",
    ...
    assert picked[:2] == ["1.6 Клиническая картина", "1.4 Особенности кодирования"]
    assert picked[-1] == "1.3 Эпидемиология"
