-- Rebuild done_cards_metrics to surface broken cards separately.
-- Adds broken_cards and audited_cards columns; excludes broken rows from
-- token/timing aggregates (same treatment as ignored rows).

DROP VIEW IF EXISTS public.done_cards_metrics;

CREATE VIEW public.done_cards_metrics AS
WITH cards AS (
    SELECT
...
ordered_intervals AS (
    ...
    FROM cards
    WHERE ignored = FALSE AND broken = FALSE
...
active_time AS (
    SELECT
        organization_name,
        visit_date,
        sum(EXTRACT(epoch FROM finished_at - started_at)) AS wall_clock_sec
...
per_date AS (
    SELECT
        ...
        count(*) FILTER (WHERE broken  = TRUE)                               AS broken_cards,
        count(*) FILTER (WHERE ignored = FALSE AND broken = FALSE)           AS audited_cards,
        sum(token_count)   FILTER (WHERE ignored = FALSE AND broken = FALSE) AS total_tokens,
        round(avg(token_count) FILTER (WHERE ignored = FALSE AND broken = FALSE)) AS avg_tokens
    FROM cards
    GROUP BY organization_name, visit_date
)
SELECT
    ...
    per_date.broken_cards,
    per_date.audited_cards,
    per_date.total_tokens,
    per_date.avg_tokens,
    round(active_time.wall_clock_sec, 2)                                               AS wall_clock_sec,
    round(active_time.wall_clock_sec / NULLIF(per_date.audited_cards, 0)::numeric, 2)  AS avg_time_sec,
