...
Core technology: LLM-based analysis (OpenAI-compatible API) combined with RAG using **reverse HyDE** — hypothetical queries are generated *from* document chunks at ingest time and stored as embeddings; at retrieval time, a new query is embedded and matched against those stored hypothetical-query embeddings rather than the raw chunk text.
...
│   ├── query_generator.py   # generate_queries(): chunk → HypotheticalQueries (3 types)
│   ├── embed_queries.py     # embed_queries(): HypotheticalQueries → vectors (async)
...
│       └── searches.py      # search_fact / search_procedural / search_constraint
...
`manifest.csv` + `pdfs/` → `load_documents()` → chunks → `generate_queries()` (3 hypothetical query types: fact / procedural / constraint) → `embed_queries()` → insert into `docs` table with three embedding columns.
...
- **Reverse HyDE**: at ingest time, each chunk gets three hypothetical queries generated by the LLM (`fact_query`, `procedural_query`, `constraint_query`). These are embedded and stored alongside the chunk. At retrieval time, the incoming query is embedded and matched against *those* hypothetical-query embeddings — not the chunk text directly. This improves recall for domain-specific Russian medical terminology.
