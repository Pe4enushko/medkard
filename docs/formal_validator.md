...
6. The model returns `{condition_met, violated, issue, comment}`. Python accepts a finding only when the rule condition and its requirement are both violated. The trusted `flag_code` and regulatory `source` are attached in code rather than generated by the model.
