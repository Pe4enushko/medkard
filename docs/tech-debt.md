...
| `contains` объединяет прямое и обратное вхождение через `OR`, из-за чего в полное сканирование уходит и индексируемая половина: 99 % стоимости каскада промаха. Разделить на два запроса; отдельно — хранимая нормализованная колонка вместо индексов по выражению (замер: `docs/grls-search-cost-2026-08-23.md`) | `src/storage/grls_storage.py:41-49` |
| Реестр БАД остаётся последней веткой каскада: «не нашли лекарство» превращается в «похоже, это БАД» без отдельного подтверждения. Это второй приоритет их же отчёта от 21.08, и уровни совпадения его не закрывают | `src/grls/lookup.py:31-35`, `src/grls/format.py` |
| Уровень `contains` не отличает солевую форму от другого вещества с общей основой («Эналаприл» ⊂ «Эналаприлат»). Лечится только словарём веществ; пока прикрыто тем, что уровень виден в ответе | `src/grls/match.py` |
| `validate_visit` (проверка всей карты одним запросом) в проде не вызывается: формальный контур ходит только через `validate_rule`. Живёт ради живых тестов `tests/test_validations.py`. Снимать вместе с остальным мёртвым кодом | `src/LLM/validations.py:96-129` |
