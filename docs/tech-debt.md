...
| Диспансерный и профилактический приём различаются в 804н, но оба дают `PROPHYLACTIC` | `validator._CODE_RULES` | 50 из 93 приёмов `B04` в приказе — диспансерные. На таком приёме срабатывают четыре правила 404н про объём ПМО (`ПРОФ_ВЗРОСЛЫЙ_*`), которым там не место, а правила 168н/192н — те, что как раз про диспансерное наблюдение, — объявлены на `primary`/`repeat` и не срабатывают. Таблица уже различает окончания `.001` и `.002`, поэтому разведение — это одна строка в `_CODE_RULES` плюс новый ключ `visit_types` в `rules.json` и решение, какие правила за ним следуют |
