...
| Диспансерный и профилактический приём различаются в 804н (`B04.*.001` против `B04.*.002`), но оба дают `PROPHYLACTIC` | `src/audit/formal_structure/nmu_visit_types.json`, `VisitType` | Правила 168н/192н объявлены для `primary`/`repeat`, поэтому на диспансерном приёме — там, где им место, — не срабатывают. Словарь уже хранит исходное наименование, различить можно без перегенерации |
