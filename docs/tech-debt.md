...
| Диспансерный и профилактический приём различаются в 804н, но оба дают `PROPHYLACTIC` | `validator._NMU_KIND_TO_VISIT_TYPE`, `src/audit/formal_structure/nmu_services.json` | 50 из 93 приёмов `B04` в приказе — диспансерные. На таком приёме срабатывают четыре правила 404н про объём ПМО (`ПРОФ_ВЗРОСЛЫЙ_*`), которым там не место, а правила 168н/192н — те, что как раз про диспансерное наблюдение, — объявлены на `primary`/`repeat` и не срабатывают. Справочник различие хранит (`kind`), разведение стоит одной строки в валидаторе плюс нового ключа `visit_types` в `rules.json` |
