...
Сравнение прогонов: детерминированный слой закрыт снимком
(`docs/deterministic-snapshot.md`), стохастический — нет. Для него нужен прогон
одной ревизии дважды (уровень шума) и ручная разметка замечаний.
