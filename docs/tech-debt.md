...
| SQL `grls_norm()` и питоновский `normalize_query()` расходятся на неразрывном пробеле: `regexp_replace('\\s+')` в Postgres его не схлопывает, `str.split()` в питоне схлопывает. Тест `test_grls_norm_parity_with_python` это ловит и КРАСЕН на dev — он требует БД и на этой машине никогда не гонялся. С хранимой колонкой расхождение фиксируется при записи | `migrations/028_grls_registry.sql:10-18`, `src/grls/normalize.py:121-123` |
| Пробелы вокруг «+» нормализация не трогает: «амоксициллин + клавулановая кислота» (как пишет врач) против «амоксициллин+клавулановая кислота» (как в реестре) даёт уровень fuzzy, то есть «препарат не опознан» на ровном месте. Лечится либо правилом нормализации разделителей, либо ветвью `tsv @@` (её замер — 0,1 % каскада) | `src/grls/normalize.py:121-123`, `src/grls/match.py` |
