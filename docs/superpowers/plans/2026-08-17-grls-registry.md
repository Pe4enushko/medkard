...
|---|---|
| `migrations/028_grls_registry.sql` | таблицы `grls_registry`/`grls_imports`, `grls_norm()`, индексы, `COMMENT ON COLUMN`, `DROP TABLE drugs` |
...
| `tests/test_migration_028_grls.py`, `test_grls_normalize.py`, `test_grls_status.py`, `test_grls_parser.py`, `test_grls_dump.py`, `test_grls_storage.py` (стенд), `test_import_grls_script.py`, `test_grls_format.py`, `test_search_medicine_tool.py` | тесты |
...
**Files:**
- Create: `migrations/028_grls_registry.sql`
- Test: `tests/test_migration_028_grls.py`
...
```python
# tests/test_migration_028_grls.py
...
Run: `pytest tests/test_migration_028_grls.py -q`
...
```bash
git add migrations/028_grls_registry.sql tests/test_migration_028_grls.py
git commit -m "feat(grls): migration 028 — grls_registry/grls_imports, grls_norm(), drop drugs"
```
...
@dataclass
class GrlsRecord:
    """One row of a GRLS status sheet (see migration 028)."""
...
Run: `pytest tests/test_search_medicine_tool.py tests/test_grls_*.py tests/test_migration_028_grls.py tests/test_import_grls_script.py -q`
Expected: все passed.
...
1. `bash migrations/migrate.sh` → 028 применилась (в `schema_migrations` есть `028_grls_registry.sql`; `\d grls_registry` показывает 23 колонки; `SELECT grls_norm('"ЭФКУРИЯ®"')` → `эфкурия`; `\dt drugs` — таблицы нет).
...
Run: `pytest tests/test_grls_*.py tests/test_migration_028_grls.py tests/test_import_grls_script.py tests/test_search_medicine_tool.py -q`
