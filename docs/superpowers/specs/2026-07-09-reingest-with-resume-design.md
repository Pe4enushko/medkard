...
- `--data-dir <dir>` — папка с PDF (и `manifest.csv` внутри, если не задан `--manifest-path`). Без
  флага — проектная раскладка (`pdfs/` + `resources/manifest.csv`).
- `--manifest-path <file>` — явный путь к `manifest.csv` (перебивает `--data-dir/manifest.csv`; манифест
  часто лежит отдельно от PDF).
