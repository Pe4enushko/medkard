...
Already-ingested files (identified by file_id) are skipped automatically.
Progress and errors are written to both stdout and logs/ingest-pdfs.log.
"""
...
async def main() -> None:
    ...
    async with DocsStorage() as storage:
        ...
        if ingested:
            log.info("Skipping %d already-ingested file(s): %s", len(ingested), sorted(ingested))
        ...
        for reader in readers:
            ...
            for chunk in reader.iter_chunks():
                ...
                try:
                    ...
                    total_errors += 1
                    log.error(
                        "Failed to insert chunk from %s (page %s): %s",
                        file_id,
