...
_MAX_HYPOTHESES = 3
# Разделы одной рекомендации, уезжающие в контекст судьи. Бюджет нужен ради
# худшего случая: один раздел «Этиология и патогенез» в корпусе весит 13 491
# символ и в одиночку обрезал бы всё, что за ним.
#
# Прежние 8000 были взяты с потолка и стерегли не то: на прогоне 2026-08-23
# вызов судьи стоил 3 828 токенов при медиане, а первый этап — 31 839, то есть
# лимит экономил на вызове, который в восемь раз дешевле обязательного.
_SECTION_CHARS_MAX = 30000
_MIN_CONFIDENCE = 8
...
# Разделы, различающие нозологии, В ПОРЯДКЕ ПОЛЕЗНОСТИ для вопроса судьи, а не
# по номеру. Порядок и есть суть: он же порядок чтения, поэтому бюджет символов
# обрезает наименее нужный раздел, а не самый нужный.
#
# Номер надёжнее названия: у клинрека 748_2 раздел зовётся "1.6 \nКлиническая" —
# имя обрезано при разборе PDF, и по слову «картина» он не находится вовсе.
_SECTION_NUMBERS: tuple[str, ...] = ("1.6", "1.4", "1.5", "1.1")
# «критери» + «диагноз» вместе: «Критерии оценки качества медицинской помощи»
# есть у 16 клинреков из 17 и судье не нужен — он про аудит лечения, не про
# нозологию. На этом корпусе пара не совпала ни разу, но раздел «Критерии
# установления диагноза» по смыслу ровно то, чем различают нозологии, и
# держать слово дешевле, чем потерять его на клинреке за пределами выборки.
...
def _pick_sections(sections: list[str]) -> list[str]:
    """Разделы для чтения — по полезности для судьи, без обращения к модели.

    Отбор по номеру подряд («всё, что начинается на 1.», первые пять») ронял
    ровно тот раздел, ради которого судья и зовётся: у 24 клинреков из 30 есть
    1.1–1.6, лимит выбирался на 1.1–1.5, и «1.6 Клиническая картина» не
    доходила никогда. Судью при этом спрашивают, подходит ли код клинической
    картине пациента, а вместо неё он читал эпидемиологию — «средний возраст
    наступления менопаузы 48,8 лет», по которой о пациенте не скажешь ничего.

    Замер на 17 настоящих оглавлениях прогона 2026-08-23: клиническая картина
    доходила до судьи у одного клинрека, стало — у четырнадцати.
    """
    ranked: list[tuple[int, int, int, str]] = []
    for position, section in enumerate(sections):
        flat = " ".join(section.lower().split())
        number = next(
            (i for i, prefix in enumerate(_SECTION_NUMBERS)
             if flat == prefix or flat.startswith(prefix + " ")),
            None,
        )
        keyword = next(
            (i for i, words in enumerate(_SECTION_KEYWORDS)
             if all(word in flat for word in words)),
            None,
        )
        if number is not None:
            ranked.append((0, number, position, section))
        elif keyword is not None:
            ranked.append((1, keyword, position, section))
        elif flat.startswith("1."):
            ranked.append((2, 0, position, section))
    ranked.sort()
    return [section for *_, section in ranked[:_SECTIONS_PER_GUIDELINE]]
